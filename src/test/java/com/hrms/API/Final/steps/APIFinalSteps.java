...
public class APIFinalSteps extends CommonMethods {
	...
	public void a_request_is_prepared_to_create_an_employee() {
		/** Way to send a payload with request */
...
//				.body(APIPayloadCommonMethods.createEmployeePayload()).log().all();
		/** Better way to send a payload with request */
//		request = given().header("Authorization", GenerateTokenSteps.token).header("Content-Type", "application/json")
...
//		 APIPayloadCommonMethods.createEmployeeBody());
	...
	public void the_status_code_for_retrieven_the_created_employee_is(int statusCodeRetrievingEmployee) {
		response.then().assertThat().statusCode(statusCodeRetrievingEmployee);

	}

	@Then("the retrieved employee ID {string} matches the globally stored employee ID")
	public void the_retrieved_employee_ID_matches_the_globally_stored_employee_ID(String id) {
